...
export class ListComponent implements OnDestroy{
    ...
    ngOnInit() {
            ...
            dataSource: {
                transport: {
                    read: function(f) {
                        ...
                        this.sub2=__this.authService.AuthGet(`${environment.sellerContractApi}api/SXContract`, f.data).subscribe(function (res) {
                            f.success(res.json());
                        });
                    }
                },
        ...
        $("#gridContracts").on("click", ".eidt", function () {
            ...
            __this.router.navigate([`/sale/contracts/shanxi/edit/${id}`]);
        });
        $("#gridContracts").on("click", ".detail", function () {
            var id = $(this).attr("val");
            __this.router.navigate([`/sale/contracts/shanxi/history/${id}`]);
        });
        ...
        var deleteList = {
            ...
            onOKDeleteIt: function() {
                ...
                this.sub3=__this.authService.AuthPost(`${environment.sellerContractApi}api/SXContract/AbandonContract/${id}`,null).subscribe(msg=>{
                    let res = msg.json();
                    if (res.result) {
