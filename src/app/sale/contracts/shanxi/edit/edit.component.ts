    ...
    mdata = {
        customer:{
            ...
            filter:(name)=>{
                ...
                if(name!=""){
                    this.sub1=this.authService.AuthGet(`${environment.sellerContractApi}api/SXContract/GetSubjectList/${name}`).subscribe((res)=> {
                        the.data = res.json();
                    });
                }else{
                    the.data.length=0;
            ...
            change:()=>{
                if(!this.mdata.customer.value || !this.mdata.customer.value.Id)return false;

                let id = this.mdata.customer.value.Id;
                let ispark = this.mdata.customer.value.IsSellerPark;
                this.sub2=this.authService.AuthGet(`${environment.sellerContractApi}api/SXContract/${id}`).subscribe((res)=> {
                    let data = res.json();

                    if (data != null) {
                        this.model.customer = data;
                        this.model.contract.SellerParkId=ispark?id:null,
                        this.model.contract.SellerSubjectId=ispark?null:id,
...
this.model.customer.IndustryCategorySecondId=data.IndustryCategorySecondId;
    ...
    submit(e) {
        ...
        let contract = {
            id:theContract.Id,
            sellerSubjectId: this.mdata.customer.value.IsSellerPark?null: theContract.SellerSubjectId,
            sellerParkId:this.mdata.customer.value.IsSellerPark?theContract.SellerParkId:null,
        ...
        this.sub10=the.authService.AuthPost(`${environment.sellerContractApi}api/Common/SubmitPackageData`, the.packageModel.data).subscribe((res)=> {
            ...
            this.sub11=the.authService.AuthPost(`${environment.sellerContractApi}api/SXContract/AddOrUpdateContract`, request).subscribe((msg)=> {
                //let msg = res.json();
                //alert(msg.);
                the.router.navigate(['/sale/contracts/shanxi/list']);
            });
        });
    }
    ...
    ngOnInit() {
        ...
        this.sub12=this.authService.AuthGet(`${environment.sellerContractApi}api/SXContract/GetContractDetail/${id}`).subscribe((res) =>{
            let m = res.json();
            let the = this.model;
            ...
            let industryCategorys =subject.industryCategory==null?null: subject.industryCategory.split(",");
            let citys = subject.city.split(",");

            this.mdata.customer.data.push({
                Id:subject.id,
                Name:subject.subjectName,
                IsSellerPark:contract.sellerParkId!=null
            });

            this.mdata.customer.value=this.mdata.customer.data[0];
            ...
            the.customer={
                ...
                TradingCenterId: 34,
                IndustryCategoryFirstId: subject.industryCategory==null?null: parseInt(industryCategorys[0]),
                IndustryCategorySecondId:subject.industryCategory==null?null: parseInt(industryCategorys[1]),
                ...
                VoltageLevel: subject.voltageLevel==null?null: subject.voltageLevel.toString(),
            ...
            the.customer.IndustryCategorySecondId = subject.industryCategory==null?null:parseInt(industryCategorys[1]);
            ...
            the.contract={
                ...
                SellerSubjectId:contract.sellerSubjectId,
                SellerParkId:contract.sellerParkId
            };
