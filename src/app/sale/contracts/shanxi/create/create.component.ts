...
export class CreateComponent implements OnDestroy {
    ...
    mdata = {
        customer:{
            ...
            change:(v)=>{
                if(this.mdata.customer.value==undefined || this.mdata.customer.value.Id==null)return;

                let id = this.mdata.customer.value.Id;
                let ispark = this.mdata.customer.value.IsSellerPark;
                this.mdata.customer.IsSellerPark = ispark;
                
                this.sub2=this.authService.AuthGet(`${environment.sellerContractApi}api/SXContract/${id}/${ispark}`).subscribe((res)=> {
                    let data = res.json();
                    if (data != null) {
                        this.model.customer = data;
                        this.model.customer.Id=id;
                        if(data.VoltageLevel!=null&&data.VoltageLevel!=undefined){
    ...
    plans={
        ...
        init:()=>{
            ...
            let start = kendo.toString(contract.StartDate, "yyyy/MM/01");
            let end = kendo.toString(contract.EndDate, "yyyy/MM/01");
    ...
    submit(e) {
        ...
        let contract = {
            sellerSubjectId: this.mdata.customer.value.IsSellerPark? null:customer.Id,
            sellerParkId: this.mdata.customer.value.IsSellerPark? customer.Id : null,
        ...
        this.sub10=the.authService.AuthPost(`${environment.sellerContractApi}api/Common/SubmitPackageData`, the.packageModel.data).subscribe((res)=> {
            ...
            this.sub11=the.authService.AuthPost(`${environment.sellerContractApi}api/SXContract/AddOrUpdateContract`, request).subscribe((smsg)=> {
                let msg = smsg.json();
                if(!msg.result){
                    alert(msg.msg);
                }
                ...
                for (var i = 0; i < e.files.length; i++) {
                    ...
                    if (file) {
                        ...
                        reader.onloadend = function () {
                            $("<div class='product'><img src=" + this.result + " id=" + fileId + " /><i class='iconfont icon-guanbipsd'></i></div>").appendTo($("#products"));
                        };
