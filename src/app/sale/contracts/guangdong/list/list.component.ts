...
export class ListComponent implements OnDestroy{
    ...
    setDay = {
        ...
        submit: (e) => {
            ...
            let __this = this;
            var day = this.setDay.form.value["day"];

            this.sub1=this.authService.AuthPost(`${environment.sellerContractApi}api/Common/SetReportingPeriod/${day}`, null).subscribe(function (msg) {
                var res = msg.json();
                if (res.result) {
                    alert("设置成功！");
                    __this.setDay.close();
    ...
    ngOnInit() {
        ...
        var filter = {
            ...
            advance: function() {
                ...
                if ($("#eq4").is(':checked') && (!$("#eq5").is(':checked'))) {
                    filters3.push(`EndDate~gt~'${new Date()}'`);
                    filters3.push(`EndDate2~lt~'${new Date()}'`);
                }
            ...
            dataSource: {
                transport: {
                    read: function(f) {
                        ...
                        this.sub2=__this.authService.AuthGet(`${environment.sellerContractApi}api/GDContract`, f.data).subscribe(function (res) {
                            f.success(res.json());
                        });
                    }
                },
            ...
            columns: [
                {
                    ...
                    template: function (item) {
                        ...
                        var planRatef = kendo.parseFloat(planRate);
                        var tradeRatef = kendo.parseFloat(tradeRate);
        ...
        $("#iSearch").off().on("click", function () {
            var grid = $("#gridContracts").data("kendoGrid");
            filter.mode = "fast";
        ...
        $("#gridContracts").on("click", ".eidt", function () {
            ...
            __this.router.navigate([`/sale/contracts/guangdong/edit/${id}`]);
        });
        $("#gridContracts").on("click", ".detail", function () {
            var id = $(this).attr("val");
            __this.router.navigate([`/sale/contracts/guangdong/history/${id}`]);
        });
        ...
        function resizeFunction() {
            var $list = $(".ehart-box");
            $list.each(function() {
                var $willBuy = $(this).find(".will-buy");
                var $bought = $(this).find(".bought");
                var $used = $(this).find(".used");
                var w = $willBuy.find(".echart-box span").width();
        ...
        var deleteList = {
            ...
            setDialog: function() {
                var delTip = '<div class="text-center dialog-box">'
            ...
            onOKDeleteIt: function() {
                var id = deleteList.deleteNode.attr("dataid");
                this.sub3=__this.authService.AuthPost(`${environment.sellerContractApi}api/GDContract/AbandonContract/${id}`,null).subscribe(msg=>{
                    let res = msg.json();
                    if (res.result) {
            ...
            bindEvent: function() {
                ...
                $(document).off("click", ".close").on("click", ".close", function() {
                    __this.dialogDeleteList.data("kendoDialog").open();
                    __this.deleteNode = $(this);
                })
            }
        }
        deleteList.init();
