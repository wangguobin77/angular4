...
export class HistoryComponent implements OnDestroy{
    ...
    submit(e) {
        ...
        this.sub1=_this.authService.AuthPost(`${environment.sellerContractApi}api/GDContract/UpdateBidPlanAmount`, _this.bidAmountModel).subscribe(function (res) {
            //let msg = res.json();
            //alert(msg.);
            _this.refresh();
        });
    ...
    ngOnInit() {
        ...
        _this.detailUrl = `/sale/contracts/guangdong/detail/${id}`;
        ...
        //合约概况
        this.sub2=_this.authService.AuthGet(`${environment.sellerContractApi}api/GDContract/GetContractInfo/${id}`).subscribe(function (res) {
        ...
        let loadGrid = function () {
            ...
            if (v == 1) {
                arr = [
                    ...
                    //{ field: "longUnitPrice", title: "长协价差<span class='size-12'>(厘/千瓦时)</span>" },
                ]
            }
            else if (v == 2) {
                arr = [
            ...
            $("#grid").kendoGrid({
                dataSource: {
                    transport: {
                        read: function (f) {
                            this.sub3=_this.authService.AuthGet(`${environment.sellerContractApi}api/GDContract/GetContractPlans/${id}`).subscribe(function (res) {
                                f.success(res.json());
                            });
                        }
                    },
