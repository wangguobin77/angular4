...
export class DetailComponent {
  ...
  changeCustomer = {
    ...
    open: (id) => {
      ...
      the.customerId=id;
      the.opened = true;
      this.sub.push(this.auth.get(`${environment.sellerCRMApi}api/PerformanceGD/GetUserInfo`).subscribe(res => {
        let data = res.json();
        the.customers=data;
      }));
    },
    save:()=>{
      ...
      this.sub.push(this.auth.post(`${environment.sellerCRMApi}api/PerformanceGD/TransferSalesForApi`, req).subscribe(res => {
        let msg = res.json();
        alert('转移成功');
        the.opened=false;
