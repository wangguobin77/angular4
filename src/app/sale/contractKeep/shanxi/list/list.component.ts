...
if (filters1.length !== 0) {
            ...
            dataSource: {
                transport: {
                    read: function (f) {
                        ...
                        paras.params = f.data;
                        _the.sub1=_the.authService.AuthGet(`${environment.sellerContractApi}api/SXContractFiling/SXGetContrant`, f.data).subscribe(function (res) {
                            f.success(res.json());
                        });
                    }
                },
        ...
        $("#grid").off("click", "a.color-exactGreen").on("click", "a.color-exactGreen", function () {
            ...
            _the.router.navigate([`/sale/contractkeep/shanxi/detail/${date}`]);
        });
