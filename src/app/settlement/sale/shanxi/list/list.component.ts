...
$("#dateyear").kendoDropDownList({
    dataSource: {
        transport: {
            read: function(f) {
                //f.success([2017,2027]);
                the.sub2=the.authService.AuthGet(`${api}SXPurchaseSettlement/GetSettlementYear`).subscribe((res)=> {
                    f.success(res.json());
                });
            },
        }
...
function resizeFunction(CustomerProfitsSum, DeviationPunishleftAmountSum, CustomerBenefitsSum) {
    var $list = $(".chart-box-r");
    $list.each(function () {
        var arr = new Array();
        arr.push(CustomerProfitsSum);
        arr.push(DeviationPunishleftAmountSum);
        ...
        if (arrMax != 0) {
            ...
            $bought.find(".echart-box span").width((DeviationPunishleftAmountSum / arrMax * 100) + "%");
...
function getGrid(SubjectName, UsedPowerType, Year, Month) {
    ...
    the.sub3=the.authService.AuthPost(`${api}SXPurchaseSettlement/GetCustomerSettlement`,request).subscribe(res=>{
        ...
        $(".DeviationPunishleftAmountSum").text(respdata.DeviationPunishleftAmountSum);
        $(".CustomerBenefitsSum").text(respdata.CustomerBenefitsSum);
        resizeFunction(respdata.CustomerProfitsSum, respdata.DeviationPunishleftAmountSum, respdata.CustomerBenefitsSum);
        //left pie
        $(".TotalDeviationAmount").html(respdata.DeviationPunishAmountSum + '<span class="size-14">元</span>');
        ...
        $("#grid").kendoGrid({
            ...
            columns: [
                ...
                { field: "TradeAmount", title: "成交电量<p>(万千瓦时)</p>" },
                { field: "ActualUsedAmount", title: "实际用电量<p>(万千瓦时)</p>" },
                { field: "DeviationAmount", title: "偏差电量<p>(万千瓦时)</p>" },
