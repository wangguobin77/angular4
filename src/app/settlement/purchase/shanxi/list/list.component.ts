...
; (function () {
    win.tcId = 6; //$("#hdTc").val();
    win.tcName = "山西交易中心";//$("#hdTc").prop("name");
    ...
    var _filter = {
        ...
        search: function () {
            var _this = this;
            the.sub1=the.authService.AuthGet(`${api}sxpurchasesettlement/getreport/${_this.data.year}`).subscribe((res)=> {
                let data = res.json();
                win.initChart(data);
...
; (function () {
    ...
    var _chart = {
        ...
        creatLeftChart: function () {
            ...
            $("#spBenefits").text(kendo.toString(total.benefitsYuan, "n"));
            $("#spPunishment").text(kendo.toString(total.punishmentYuan, "n"));
            $("#spProfits").text(kendo.toString(total.profitsYuan, "n"));

            var punishmentRate = kendo.toString(total.punishmentYuan / total.benefitsYuan * 100, "n");
            var profitsRate = kendo.toString(total.profitsYuan / total.benefitsYuan * 100, "n");
        ...
        creatRightChart: function () {
            ...
            $("#chartLine").kendoChart({
                ...
                series: [
                    {
                        name: "收益",
                        field: "benefitsGeneral",
                        color: "#333333"
                    },
                    {
                        name: "偏差惩罚",
                        field: "deviationPunishAmount",
                        color: "#99cc33"
                    },
                    {
                        name: "净收益",
                        field: "benefitsAmount",
                        color: "#3399ff"
                    }
                ],
                valueAxis: {
        ...
        load: function () {
            ...
            $("#grid").kendoGrid({
                ...
                columns: [
                    ...
                    {
                        field: "plans", title: "申报电量<p>(万千瓦时)</p>",
                        template: function (item) {
                            if (!item.plans) return "—";
                            var arr = [];
                            $.each(item.plans, function (i, m) {
                    ...
                    {
                        field: "plans", title: "成交电量<p>(万千瓦时)</p>",
                        template: function (item) {
                            if (!item.plans) return "—";
                            var arr = [];
                            $.each(item.plans, function (i, m) {
                    ...
                    { field: "benefitsGeneralDisplay", title: "收益<p>(元)</p>" },
                    //{ field: "edeviationBalanceDisplay", title: "偏差平衡<p>(元)</p>" },
                    { field: "deviationPunishAmountDisplay", title: "偏差惩罚<p>(元)</p>" },
                    { field: "benefitsAmountDisplay", title: "公司净收益<p>(元)</p>" },
...
; (function () {
    var _settle = {
        ...
        add: function () {
            ...
            the.sub2=the.authService.AuthPost(`${api}sxpurchasesettlement/addsettlement/`, input).subscribe((res)=> {
                alert("结算成功");
                win.refresh();
        ...
        load: function () {
            ...
            the.sub3=the.authService.AuthPost(`${api}sxpurchasesettlement/opensettlement/`, _settle.data.open).subscribe((res)=> {
                let data = res.json();
...
; (function () {
    var _settle = {
        ...
        load: function () {
            ...
            the.sub4=the.authService.AuthGet(`${api}sxpurchasesettlement/lookvouchers/`, _settle.data.open).subscribe((res)=> {
                let data = res.json();
...
; (function () {
    ...
    var _settle = {
        ...
        add: function () {
            ...
            the.sub5=the.authService.AuthPost(`${api}sxpurchasesettlement/addsettlement/`, input).subscribe((res)=> {
                alert("结算成功");
                win.refresh();
                $("#endDialog").data("kendoDialog").close();
            });
        ...
        load: function () {
            ...
            the.sub6=the.authService.AuthPost(`${api}sxpurchasesettlement/opensettlement/`, _settle.data.open).subscribe((res)=> {
                let data = res.json();
                _settle.data.opened = data;
                var unPaid = kendo.toString(data.unPaidPrice, "n");
