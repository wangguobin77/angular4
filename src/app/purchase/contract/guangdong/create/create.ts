...
export class GDContractCreate implements OnDestroy {
	...
	onsubmit(e, m) {
		...
		this.sub3=this.auth.post(`${environment.sellerEnergyPurchaseApi}api/GDEnergyContract/EnergyContractSave`, this.postData, this.options).subscribe((res) => {
			console.log(res);
			let result=res.json();
			if(result.result){
				this.router.navigate(['/purchase/contract/guangdong/list']);
			}else{
...
onDateChange(e){
	...
	ngOnInit() {
		...
		if(this.id != null && this.id.length > 0 && this.id != "0"){
           	this.sub4=this.auth.get(`${environment.sellerEnergyPurchaseApi}api/GDEnergyContract/GetAddInitData/${this.id}`).subscribe((res)=>{
