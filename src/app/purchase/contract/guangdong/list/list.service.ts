...
export abstract class NorthwindService extends BehaviorSubject<GridDataResult> implements OnDestroy {
    ...
    private fetch(tableName: string, state: any): Observable<GridDataResult> {
        ...
        const totleUrl = `${environment.sellerEnergyPurchaseApi}api/GDEnergyContract/GetData?${queryStr}`;
...
@Injectable()
export class GDDataService extends NorthwindService {
    constructor(http: Http,auth:AuthHttpService) { super(http, "EnergyContract", auth); }
}