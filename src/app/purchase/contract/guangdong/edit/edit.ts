...
export class GDContractEdit implements OnDestroy {
	...
	onsubmit(e, m) {
		...
		this.sub3=this.auth.post(`${environment.sellerEnergyPurchaseApi}api/GDEnergyContract/EnergyContractSave`, this.postData).subscribe((res) => {
			console.log(res);
			let result=res.json();
			if(result.result){
				this.router.navigate(['/purchase/contract/guangdong/detail',this.id]);
			}else{
...
onDateChange(e){
	...
	ngOnInit() {
		...
		this.sub4=this.auth.get(`${environment.sellerEnergyPurchaseApi}api/GDEnergyContract/GetDetail/${this.id}`).subscribe((res)=>{
			let result = res.json();
