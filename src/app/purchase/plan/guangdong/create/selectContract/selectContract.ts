...
export class SelectContract implements OnDestroy{
    ...
    searchPost(){
        ...
        this.sub1=this.auth.post(`${environment.sellerEnergyPurchaseApi}api/GDPurchasePlan/GetContactUseList`,this.dataFilter).subscribe((res)=>{
        	console.log(res);
        	let result = res.json();
        	for(let k of result){
        		k.isChecked = false;
    ...
    submitTotleCount(){
    	...
    	this.router.navigate(['/purchase/plan/guangdong/create']);
    }
    ngOnInit(){
    	this.searchPost();
