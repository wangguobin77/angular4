...
export class CreateComponent implements OnDestroy{
    ...
    submit(e,m){
        ...
        this.sub1=this.auth.post(`${environment.sellerEnergyPurchaseApi}api/GDPurchasePlan/Add`,postValue)
        .subscribe((res)=>{
            let result = res.json();
            if(result.result){
                ...
                this.router.navigate(['/purchase/plan/guangdong/list']);
            }else{
    ...
    selectContract(){
        if(this.timeSelected){
            ...
            this.router.navigate(['/purchase/plan/guangdong/create/selectContract']);
        }else{
            if(!this.timeSelected){
