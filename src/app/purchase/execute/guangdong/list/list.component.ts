    ...
    ngOnInit() {
        ...
        function GetPurchaseList(statusVal,purchasePlanType)
        {
            $("#grid").kendoGrid({
                    ...
                    transport: {
                        read:function(f) {
                        this.sub1=_this.auth.get(`${environment.sellerEnergyPurchaseApi}api/GDPurchaseExecute/GetPurchaseExecuteInfo/${statusVal}`).subscribe(function (res) {
                            console.log(res);
                            f.success(res.json());
                        });
                    } 
                    ...
                    template: function(item){
                        ...
                        if (statusVal == 1) {
                            str1 = `<div class="td-l-box">
                                ...
                                <span>${item.dealAmount}万千瓦时</span>
                                </p>
                                <p>
                                <span class="color-9">交易价差：</span>
                                <span>${item.dealUnitPrice}元/千万时</span>
                                </p>
                                </div>`
        ...
        $("#grid").off("click", ".exec1").on("click", ".exec1", function () {
            _this.router.navigate(['/purchase/execute/guangdong/detail',$(this).data("id")]);
        })
        //竞价采购计划执行
        $("#grid").off("click", ".exec2").on("click", ".exec2", function () {
            _this.router.navigate(['/purchase/execute/guangdong/bid-detail',$(this).data("id")]);
        })
        //长协已执行的采购计划 详情
         $("#grid").off("click", ".detail1").on("click", ".detail1", function () {
            _this.router.navigate(['/purchase/execute/guangdong/check/',$(this).data("id")]); //
        });
        //竞价已执行的采购计划 详情
         $("#grid").off("click", ".detail2").on("click", ".detail2", function () {
            _this.router.navigate(['/purchase/execute/guangdong/bid-check/',$(this).data("id")]); //
        });
    }
        ngOnDestroy(): void {
