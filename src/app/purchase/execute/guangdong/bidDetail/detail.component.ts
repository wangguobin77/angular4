...
export class DetailComponent implements OnDestroy{
  ...
  purchaseExecuteFiles = [];
    ...
    submit(e,m){
        ...
        let postValue = {purchaseExecuteFiles:[],purchasePlanId:"",purchasePlanType:2,dealAmount:0,dealUnitPrice:0};
        postValue.purchasePlanId = this.model.id;
        postValue.purchasePlanType = this.model.purchasePlanType;
        postValue.dealAmount =this.eleBought;
        postValue.dealUnitPrice = this.price;
        ...
        this.sub1=this.auth.post(`${environment.sellerEnergyPurchaseApi}api/GDPurchaseExecute`,postValue,this.options).subscribe((res)=>{
            let result = res.json();
             console.log(result);
            if(result.result){
                 this.router.navigate(['/purchase/execute/guangdong/bid-check/',this.model.id]);
            }
        })
    }
    ngOnInit() {
        ...
        this.sub2=this.auth.get(`${environment.sellerEnergyPurchaseApi}api/GDPurchaseExecute/GetModelInfo/${id}`).subscribe( (res) =>{
            let result = res.json();
            this.model = result;
        });
    }
