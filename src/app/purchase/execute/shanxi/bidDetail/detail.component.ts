...
export class DetailComponent implements OnDestroy{
  ...
  purchaseExecuteFiles = [];
    ...
    submit(e,m){
        ...
        let postValue = {purchaseExecuteFiles:[],relationCustomer:[],purchasePlanId:""};
        postValue.purchasePlanId = this.model.id;
        postValue.relationCustomer = this.gridData;
        postValue.purchaseExecuteFiles = this.purchaseExecuteFiles;
        console.log(postValue);
        this.sub1=this.auth.post(`${environment.sellerEnergyPurchaseApi}api/SXPurchaseExecute`,postValue,this.options).subscribe((res)=>{
            let result = res.json();
             console.log(result);
            if(result.result){
                 this.router.navigate(['/purchase/execute/shanxi/bid-check/',this.model.id]);
            }
        })
    }
    onValueChange(e){
        ...
        this.totleCount = 0;
        this.gridData.forEach(f =>{this.totleCount += f.dealAmount});
    }
    ngOnInit() {
        
        let id = this.route.snapshot.params['id'];
        this.sub2=this.auth.get(`${environment.sellerEnergyPurchaseApi}api/SXPurchaseExecute/GetModelInfo/${id}`).subscribe( (res) =>{
            let result = res.json();
            this.model = result;
            this.gridData = result.planRelationCustomer;
        });
    }
