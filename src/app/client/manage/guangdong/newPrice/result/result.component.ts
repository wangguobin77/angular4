...
export class Result{
	...
	submit(){
		...
		this.sub2=this.auth.post(`${environment.sellerCRMApi}api/GDQuoteProgram/CreateProgramme`,postData).subscribe((res)=>{
			let result = res.json();
			if(result.result) {
				this.opened = true;
			}
	...
	ngOnInit(){
		...
		this.sub1=this.auth.post(`${environment.sellerCRMApi}api/GDQuoteProgram/GetPakageDetailById`,postData).subscribe((res)=>{
			let result = res.json();
			for(let k of result){
				k.isSelect = false;
			}
