...
export class Client implements OnInit, OnDestroy {
    ...
    submitBaseInfo() {
        const clientUrl = `${environment.sellerCRMApi}api/CustomerGD/ModifyCustomer/`;
        this.sub1 = this.auth.post(clientUrl, this.editInfo)
            .subscribe((res) => {
    ...
    getTotle() {
        ...
        const url = `api/customerGD/detail/?id=${this.id}`;
    ...
    ngOnInit() {
        ...
        this.sub5 = this.auth.get(`${environment.sellerCRMApi}api/CertGD/GetAptitudeInformation/?sellerSubjectId=${this.id}`)
            .subscribe((res) => {
                const result = res.json();
                if (result.length !== 0) {
                    this.aptIsNew = false;
        ...
        // 证照列表
        this.sub6 = this.auth.get(`${environment.sellerCRMApi}api/CertGD/GetCertificatesInformation/?sellerSubjectId=${this.id}`)
            .subscribe((res) => {
    ...
    removeHandler({ dataItem }) {
        this.sub7 = this.auth.post(`${environment.sellerCRMApi}api/CustomerGD/RemoveCustomerContacts`, { 'id': dataItem.id })
            .subscribe((res) => {
                const result = res.json();
    ...
    onContactDialogSave() {
        ...
        this.sub8 = this.auth.post(`${environment.sellerCRMApi}api/CustomerGD/ModifyCustomerContacts`, postData)
            .subscribe((res) => {
                const result = res.json();
                if (result.return_code === 'SUCCESS') {
                    this.active = false;
