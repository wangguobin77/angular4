...
export class ClientSale implements OnInit, OnDestroy, AfterViewInit {
    ...
    onSave(e, m) {
        ...
        this.sub1 = this.auth.post(`${environment.sellerCRMApi}api/CustomerGD/CreateCustomerTrack`, postData)
            .subscribe((res) => {
    ...
    onTransferSave() {
        this.sub2 = this.auth.post(`${environment.sellerCRMApi}api/CustomerGD/TransferCustomer`, this.transferData)
            .subscribe((res) => {
                if (res.json().return_code === 'SUCCESS') {
                    this.getSalePeople();
    ...
    getSalePeople() {
        ...
        this.sub3 = this.auth.get(
            `${environment.sellerCRMApi}api/CustomerGD/GetSalerInformation/?sellerSubjectID=${this.route.snapshot.params['id']}`)
            .subscribe((res) => {
    ...
    ngOnInit() {
        ...
        // 销售人员列表
        this.sub5 = this.auth.get(`${environment.sellerCRMApi}api/CustomerGD/GetSalesList`)
            .subscribe((res) => {
                this.TransferData = res.json();
            })

        this.sub4 = this.auth.get(
            `${environment.sellerCRMApi}api/CustomerGD/GetSellerSubjectById/?sellerSubjectId=${this.route.snapshot.params['id']}`)
            .subscribe((res) => {
                const result = res.json();
    ...
    onProgressSave(m) {
        this.sub6 = this.auth.post(`${environment.sellerCRMApi}api/CustomerGD/UpdateCustomerProgressPercent`, this.progressData)
            .subscribe((res) => {
                console.log(res);
                if (res.json().return_code === 'SUCCESS') {
