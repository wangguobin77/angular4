...
export class Power implements OnDestroy, AfterViewInit {
    ...
    removeHandler({ dataItem }) {
        this.sub1 = this.auth.post(`${environment.sellerCRMApi}api/CustomerGD/RemoveCustomElectricity`, { id: dataItem.id })
            .subscribe((res) => {
    ...
    onSave(m) {
        if (this.datepickerMonth.value() === '' ||
            ...
            this.datepickerMonth.value() === undefined || !m.valid) { return; }
        const postData = {
        ...
        this.sub2 = this.auth.post(`${environment.sellerCRMApi}api/CustomerGD/ModifyCustomElectricity`, postData)
            .subscribe((res) => {
