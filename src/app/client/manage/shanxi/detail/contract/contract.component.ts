...
export class ClientContract implements OnInit, OnDestroy {
  ...
  PowerContractChange(e) {
    ...
    ngOnInit() {
        ...
        this.sub1 = this.auth.get(
            `${environment.sellerCRMApi}api/ContractManageSX/ClientContract/?sellerSubjectId=${this.route.snapshot.params['id']}`)
            .subscribe((res) => {
                   ...
                   dataSource: {
                         transport: {
                                read: function (f) {
                                    ...
                                    let totalurl = `${environment.sellerSettlementApi}api/SXPurchaseSettlement/GetCustomerSettlement`;
