...
export class Aptitude implements OnInit, OnDestroy {
    ...
    onsubmit() {
        this.sub1 = this.auth.post(`${environment.sellerCRMApi}api/CertSX/SaveApt/`, this.postData)
            .subscribe((res) => {
    ...
    ngOnInit() {
        ...
        this.sub2 = this.auth.get(`${environment.sellerCRMApi}api/CertSX/AptEdit/?sellerSubjectId=${this.pageId}`)
            .subscribe((res) => {
                const result = res.json();
                if (result.edit) {
