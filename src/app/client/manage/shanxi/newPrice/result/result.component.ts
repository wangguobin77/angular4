...
export class Result{
	...
	isSelectIt(event,listItem){
		// dataItem.isSelect,dataItem.salePackagesId
	...
	submit(){
		...
		this.sub2=this.auth.post(`${environment.sellerCRMApi}api/SXQuoteProgram/CreateProgramme`,postData).subscribe((res)=>{
			let result = res.json();
			if(result.result) {
				this.opened = true;
			}
	...
	ngOnInit(){
		...
		this.sub1=this.auth.post(`${environment.sellerCRMApi}api/SXQuoteProgram/GetPakageDetailById`,postData).subscribe((res)=>{
			let result = res.json();
			for(let k of result){
				k.isSelect = false;
			}
