...
export class ClientSale implements AfterViewInit, OnDestroy, OnInit {
    private gridData = [];
    ...
    onSave(e, m) {
        ...
        this.sub1 = this.auth.post(`${environment.sellerCRMApi}api/Customer/CreateCustomerTrack`, postData)
            .subscribe((res) => {
    ...
    onTransferSave() {
        this.sub2 = this.auth.post(`${environment.sellerCRMApi}api/Customer/TransferCustomer`, this.transferData)
            .subscribe((res) => {
                if (res.json().return_code === 'SUCCESS') {
                    this.getSalePeople();
                    this.TransferDialog = false;
    ...
    getSalePeople() {
        // 销售人员
        this.sub3 = this.auth
            .get(`${environment.sellerCRMApi}api/Customer/GetSalerInformation/?sellerSubjectID=${this.route.snapshot.params['id']}`)
            .subscribe((res) => {
    ...
    ngOnInit() {
        ...
        // 销售人员列表
        this.sub4 = this.auth.get(`${environment.sellerCRMApi}api/Customer/GetSalesList`)
            .subscribe((res) => {
                this.TransferData = res.json();
            })

        this.sub5 = this.auth
            .get(`${environment.sellerCRMApi}api/Customer/GetSellerSubjectById/?sellerSubjectId=${this.route.snapshot.params['id']}`)
            .subscribe((res) => {
                const result = res.json();
    ...
    onProgressSave(m) {
        this.sub6 = this.auth.post(`${environment.sellerCRMApi}api/Customer/UpdateCustomerProgressPercent`, this.progressData)
            .subscribe((res) => {
                if (res.json().return_code === 'SUCCESS') {
