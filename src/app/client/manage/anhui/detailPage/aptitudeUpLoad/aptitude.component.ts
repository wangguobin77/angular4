...
export class Aptitude implements OnInit, OnDestroy {
    ...
    onsubmit() {
        this.sub1 = this.auth.post(`${environment.sellerCRMApi}api/Cert/SaveApt/`, this.postData)
            .subscribe((res) => {
    ...
    ngOnInit() {
        ...
        this.sub2 = this.auth.get(`${environment.sellerCRMApi}api/Cert/AptEdit/?sellerSubjectId=${this.pageId}`)
            .subscribe((res) => {
                const result = res.json();
                if (result.edit) {
