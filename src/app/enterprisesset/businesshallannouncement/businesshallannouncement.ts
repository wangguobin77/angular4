...
@Component({
    ...
    selector: 'businesshallannouncement',
    templateUrl: './businesshallannouncement.html',
    styleUrls: ['./businesshallannouncement.scss']
})


export class businesshallannouncement implements OnInit,OnDestroy{
...
sub2;
    ...
    submit(e) {
       ...
       _this.sub1= _this.authService.AuthPost(`${environment.sellerCRMApi}api/SellerAnnouncements/DeleteAnnouncement`,
            _this.delModel).subscribe(function (res) {
                _this.refresh();
            });
    }
...
let that=this;
        $("#grid").kendoGrid({
            dataSource: {
                transport: {
                    read: function (f) {
                        ...
                        //let url = `api/SellerAnnouncements/GetAnnouncementsList`;
                        //let totalurl = `${environment.sellerCRMApi}${url}`;

                        that.sub2=that.authService.AuthGet(`${environment.sellerCRMApi}/api/SellerAnnouncements/GetAnnouncementsList?type=2`).subscribe((res) => {
                            let result = res.json();
            ...
            columns: [
                //{ field: "id", title: "id"},
                { field: "title", title: "标题", width: "28%" },
                { field: "formatPublishDate", title: "发布时间", width: "28%" },
                { field: "publishUserName", title: "发布人", width: "28%" },
                {
                    command: [

                        {
                            ...
                            click: function (e) {
                                ...
                                window.location.href = "enterprisesset/editoraddbusinesshallannouncement/" + item.id;
                            }
                        },
